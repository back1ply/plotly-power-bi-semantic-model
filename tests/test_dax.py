...
class TestFetchStartupQuery:
    def test_returns_query_for_valid_key(self):
        ...
        dax = loader.get_raw_query(QueryKey.KEY_PERFORMANCE_INDICATOR_TOTALS)
...
class TestDaxCache:
    def test_json_loaded_and_cached(self):
        """DataAnalysisExpressionsQueryLoader loads and stores data."""
        dax_path = Path(__file__).resolve().parent.parent / "queries" / "dax.json"
        loader = DataAnalysisExpressionsQueryLoader.from_path(dax_path)
        loader.get_raw_query(QueryKey.KEY_PERFORMANCE_INDICATOR_TOTALS) # Trigger load
        assert loader._loaded is True
        assert len(loader._startup) > 0
        assert QueryKey.KEY_PERFORMANCE_INDICATOR_TOTALS in loader._startup
